...
class IUXRAY(data.Dataset):
    ...
    def __get_nounphrase(self, top_k=100, file_name='count_nounphrase.json'):
        count_np = json.load(open('D:/liuman/Context-Enhanced-Framework-main/Context-Enhanced-Framework-main/tools/count_nounphrase.json', 'r'))
        sorted_count_np = sorted([(k,v) for k,v in count_np.items()], key=lambda x: x[1], reverse=True)
        top_nounphrases = [k for k,v in sorted_count_np][:top_k]
        return top_nounphrases

    def __input_data(self, binary_mode=True):
        self.__input_caption()  #self.caption
        self.__input_report()   #self.file_list, self.file_report
        self.__input_label()    #self.file_lables
        self.__filter_inputs()  #过滤上面得到的self.file_report中的数据，得到新的self.file_list, self.file_report
        self.top_np = self.__get_nounphrase()   #得到出现最频繁的前100个词
