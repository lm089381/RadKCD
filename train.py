...
def infer(data_loader, model, device='cpu', threshold=None):
    ...
    with torch.no_grad():
        prog_bar = tqdm(data_loader)    #遍历 data_loader，获取 source（输入数据）和 target（真实标签）。
...
if __name__ == "__main__":
    ...
    test_loader = data.DataLoader(test_data, batch_size=BATCH_SIZE, shuffle=False, num_workers=8, drop_last=True)
    ...
    checkpoint_path_from = 'results/iu_xray/checkpoints/{}_{}_{}_{}.pt'.format(DATASET_NAME,MODEL_NAME,BACKBONE_NAME,COMMENT)
    checkpoint_path_to = 'results/iu_xray/checkpoints/{}_{}_{}_{}.pt'.format(DATASET_NAME,MODEL_NAME,BACKBONE_NAME,COMMENT)
    ...
    if PHASE == 'TRAIN':
        ...
        plt.legend()
        plt.grid()
        image_path = 'loss_curve.png'
        plt.savefig(image_path)
        print(f'Loss curve saved to {image_path}')
        plt.show()
        ...
        out_file_ref = open('outputs/x_{}_{}_{}_{}_Ref.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT), 'w', encoding='utf-8')
        out_file_hyp = open('outputs/x_{}_{}_{}_{}_Hyp.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT), 'w', encoding='utf-8')
        out_file_lbl = open('outputs/x_{}_{}_{}_{}_Lbl.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT), 'w', encoding='utf-8')
